...
enum TransactionType: String {
    // following the model, we can set up some initial data tht we can use to display in the previews only, without calling any API.
    //If the intent of the data is for mocking only, then it's important to arrange these in the Preview Content folder. In fact any file under it won't be included in the production build.
    case debit = "debit"
    case credit = "credit"
}
...
struct Category {
    let id: Int
    let name: String
    let icon: FontAwesomeCode // import SwiftUI SwiftUIFontIcon
